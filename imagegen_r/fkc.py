...
def run_fkc_simulation_image(
    ...
    for idx, timestep in enumerate(timesteps):
        ...
        r_grad_scale = (score_norm / r_grad_norm).view(-1, 1, 1, 1)
        r_grad_scale = torch.clamp(r_grad_scale, min=0.1, max=10.0)
        r_grad_normalized = r_grad * r_grad_scale

        if diversity_enabled and historical_particles is not None:
            div_grad_norm = torch.norm(div_grad.flatten(1), dim=1, keepdim=True)
            div_grad_norm = torch.clamp(div_grad_norm, min=1e-8)
            div_grad_scale = (score_norm / div_grad_norm).view(-1, 1, 1, 1)
            div_grad_scale = torch.clamp(div_grad_scale, min=0.1, max=10.0)
            div_grad_normalized = div_grad * div_grad_scale
        else:
